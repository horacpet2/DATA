...
kde $a_1, a_2, ... a_k$ z oboru $\setR$ jsou koeficienty mnohočlenu a $x^{m}, y^{n}, ... z^{o}$ jsou proměnné mnohočlenu. V případě, že se v mnohočlenu nachází více proměnných, pak se jejich exponenty sčítají a stupeň mnohočlenu je dán největší hodnotou součtu všech mocnin daného členu: $s = m + n + ... + o$
...
\PodSekce{Součet a rozdíl mnohočlenů}
...
\PodSekce{Součin mnohočlenu a proměnné (konstanty)}
...
\PodSekce{Součin dvou mnohočlenů}
...
\PodSekce{Podíl mnohočlenu proměnnou (konstantou)}
...
\PodSekce{Podíl mnohočlenu mnohočlenem}
...
\PodSekce{Rozklad mnohočlenů pomocí vzorců}
...
\Nadpis{Racionální lomenné výrazy}

\Nadpis{Iracionální výrazy}

\Nadpis{Exponenciální výrazy}

\Nadpis{Logaritmické výrazy}

\Nadpis{Goniometrické výrazy}
...
\Nadpis{Soustavy rovnice}



\Nadpis{Soustavy nerovnic}
